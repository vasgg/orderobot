...
answer = {
    ...
    'customer_applications_reply': '🗂️ <b>На ваши заказы есть следующие заявки:</b>\n\n',
    ...
    'information': '⁉️ КАК ПОЛЬЗОВАТЬСЯ ПЛАТФОРМОЙ\n\n'
                   '1. Публикация заказов: для публикации нового заказа нажмите на кнопку [⚡️ Создать заказ], '
                   'после чего будет создан новый черновик, к которому вы сможете в любой момент вернуться.'
                   '\nКак только заполните все поля, нажмите на кнопку [💠 Опубликовать], проект будет отправлен  в публичный каталог заказов. '

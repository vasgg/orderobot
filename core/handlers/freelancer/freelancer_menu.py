...
async def fl_find_order_handler(
    ...
    orders = get_orders(user_id=user.id, mode="others", status="published")
...
async def fl_get_order_info_handler(call: types.CallbackQuery) -> None:
...
async def fl_get_customer_of_order_handler(call: types.CallbackQuery) -> None:
    order_id = int(call.data.split(":")[1])
    order = get_order(order_id)
    ...
    await call.message.answer(
        text=answer["customer_details"].format(
            ...
            get_deals(customer.id, mode="customer"),
        ),
...
async def my_account_handler(call: types.CallbackQuery, user: User) -> None:
    ...
    deals = answer["deals_as_freelancer"].format(
        get_deals(user_id=user.id, mode="freelancer")
    )
    await call.message.answer(
